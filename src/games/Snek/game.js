...
class Game {
    ...
    render() {
        ...
        for (let i = 0; i < this.applePos.length; i++) {
            ctx.fillRect(this.applePos[i].x * this.gridScale + this.gridScale / 5, this.applePos[i].y * this.gridScale + this.gridScale / 5, this.gridScale - (this.gridScale / 5) * 2, this.gridScale - (this.gridScale / 5) * 2);
        }
        ...
        for (let i = 0; i < this.trail.length; i++) {
            switch (this.trail[i].facing) {
                case "up":
                    ctx.fillRect(this.trail[i].x * this.gridScale, this.trail[i].y * this.gridScale -
                        (this.currentSubTick * this.gridScale) / 10, this.gridScale, this.gridScale);
                    break;
                case "down":
                    ctx.fillRect(this.trail[i].x * this.gridScale, this.trail[i].y * this.gridScale +
                        (this.currentSubTick * this.gridScale) / 10, this.gridScale, this.gridScale);
                    break;
                case "left":
                    ctx.fillRect(this.trail[i].x * this.gridScale -
                        (this.currentSubTick * this.gridScale) / 10, this.trail[i].y * this.gridScale, this.gridScale, this.gridScale);
                    break;
                case "right":
                    ctx.fillRect(this.trail[i].x * this.gridScale +
                        (this.currentSubTick * this.gridScale) / 10, this.trail[i].y * this.gridScale, this.gridScale, this.gridScale);
    ...
    tick() {
        ...
        for (let i = this.trail.length; i > 1; i--) {
            ...
            if (this.trail[i - 2].x > this.trail[i - 1].x &&
                this.trail[i - 2].x - 1 !== this.trail[i - 1].x)
                this.trail[i - 1].x++;
            if (this.trail[i - 2].x < this.trail[i - 1].x &&
                this.trail[i - 2].x + 1 !== this.trail[i - 1].x)
                this.trail[i - 1].x--;
            if (this.trail[i - 2].y > this.trail[i - 1].y &&
        ...
        this.applePos.forEach(apple => {
            if (this.x === apple.x && this.y === apple.y) {
                ...
                while (this.trail.some(segment => segment.x === newApple().x && segment.y === newApple().y)) {
                    newApple();
                }
